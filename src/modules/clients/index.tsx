...
function valueFor(customer: Customer, key: string) {
...
async function loadSettings() {
  try {
    return await customerManagementService.getSettings();
  } catch {
...
export function CustomersListPage() {
  ...
  const columns = useMemo(
    () => settings.table_columns.filter(column => column.visible !== false).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)),
  ...
  const load = async () => {
    ...
    try {
      ...
      const customersPayload = await customerManagementService.listCustomers({ search, page });
            ...
            {loading ? (
              <TableRow>
                <TableCell colSpan={columns.length + 1} className="h-24 text-center text-muted-foreground">
                  Carregando {settings.plural_label.toLowerCase()}
                </TableCell>
              </TableRow>
            ) : customers.length === 0 ? (
              <TableRow>
                <TableCell colSpan={columns.length + 1} className="h-24 text-center text-muted-foreground">
                  Nenhum {settings.singular_label.toLowerCase()} encontrado
                </TableCell>
              </TableRow>
            ) : customers.map(customer => (
              <TableRow key={customer.id}>
                {columns.map(column => (
                  <TableCell key={column.key}>{valueFor(customer, column.key)}</TableCell>
                ))}
