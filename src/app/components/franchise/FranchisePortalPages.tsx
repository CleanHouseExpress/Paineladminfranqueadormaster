...
export function FranchiseInventory() {
  ...
  return <Shell title="Estoque" description="Saldos por local e disponibilidade operacional da sua unidade.">
    <Table headers={['Item', 'Saldo', 'Reservado', 'Bloqueado', 'Disponível', 'Mínimo']} rows={stock.data.stock.map(row => [String(row.item_name), `${row.on_hand ?? 0} ${row.unit_of_measure}`, String(row.reserved ?? 0), String(row.blocked ?? 0), String(row.available ?? 0), settings.enable_stock_minimum ? String(row.minimum_stock) : '?'])} />
  </Shell>;
}
