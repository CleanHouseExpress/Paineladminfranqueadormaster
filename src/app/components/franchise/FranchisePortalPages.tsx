...
function Shell({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  ...
  return <div style={{ padding: 24, maxWidth: 1240, margin: '0 auto' }}>
    <div style={{ marginBottom: 18 }}><div style={{ color: '#3B82F6', fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }}>Unidade Ã‚Â· {data?.unit.name}</div><h1 style={{ margin: '4px 0 2px', color: '#0F172A', fontSize: 22 }}>{title}</h1><p style={{ margin: 0, color: '#64748B', fontSize: 13 }}>{description}</p></div>
    {children}
  </div>;
}
...
function useLoad<T>(loader: () => Promise<T>, dependencies: unknown[] = []) {
  ...
  const load = async () => {
    ...
    try { setData(await loader()); } catch { setError('NÃƒÂ£o foi possÃƒÂ­vel carregar os dados da unidade.'); }
...
export function FranchiseDashboard() {
  ...
  return <State result={result}>{data => {
    ...
    const cards = [
      ['Vendas do mÃƒÂªs', money(d.sales.net_total), TrendingUp, '#2563EB'],
    ...
    return <Shell title="Dashboard da Franquia" description="Indicadores essenciais da operaÃƒÂ§ÃƒÂ£o da sua unidade.">
...
export function FranchiseSales() {
  ...
  return <State result={result}>{response => <Shell title="Vendas" description="Pedidos e vendas registrados para sua unidade."><Table headers={['NÃƒÂºmero', 'Data', 'Cliente', 'Status', 'Pagamento', 'Total']} rows={response.data.map(row => [String(row.number ?? 'Ã¢â‚¬â€'), String(row.sale_date ?? 'Ã¢â‚¬â€'), String(row.customer_name ?? 'Ã¢â‚¬â€'), String(row.status ?? 'Ã¢â‚¬â€'), String(row.payment_status ?? 'Ã¢â‚¬â€'), <strong>{money(row.total)}</strong>])} /></Shell>}</State>;
}
...
export function FranchiseFinancial() {
  ...
  return <Shell title="Financeiro" description="Receitas e despesas vinculadas ÃƒÂ  unidade.">
    ...
    <Table headers={['DescriÃƒÂ§ÃƒÂ£o', 'Tipo', 'Categoria', 'Vencimento', 'Status', 'Valor']} rows={transactions.data.data.map(row => [String(row.description), String(row.type), String(row.category ?? 'Ã¢â‚¬â€'), String(row.due_date ?? 'Ã¢â‚¬â€'), String(row.status), money(row.amount)])} />
  </Shell>;
}
...
export function FranchiseDre() {
  ...
  return <State result={result}>{data => {
    ...
    return <Shell title="DRE Gerencial" description="Demonstrativo exclusivo da sua unidade."><Table headers={['DescriÃƒÂ§ÃƒÂ£o', 'Valor']} rows={rows.map(row => [<strong>{String(row.label)}</strong>, money(row.amount)])} /></Shell>;
  }}</State>;
}
...
export function FranchiseRoyalties() {
  ...
  return <Shell title="Royalties" description="CompetÃƒÂªncias e status de cobranÃƒÂ§a da sua unidade.">
    ...
    <Table headers={['CompetÃƒÂªncia', 'Regra', 'Base', 'Valor', 'Status']} rows={rows.data.data.map(row => [`${String(row.reference_month).padStart(2, '0')}/${row.reference_year}`, String((row.rule as Record<string, unknown>)?.name ?? 'Ã¢â‚¬â€'), money(row.base_amount), money(row.calculated_amount), String(row.status)])} />
  </Shell>;
}
...
export function FranchiseCmv() {
  ...
  return <Shell title="CMV" description="Custos de consumo e perdas da unidade."><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 16 }}>{[['Consumo', metrics.data.consumption_cost], ['Perdas', metrics.data.loss_cost], ['Ajustes', metrics.data.adjustment_cost]].map(([label, value]) => <div key={String(label)} style={{ ...card, padding: 15 }}><strong>{money(value)}</strong><span style={{ display: 'block', color: '#64748B', fontSize: 11 }}>{label}</span></div>)}</div><Table headers={['Item', 'Quantidade', 'Custo mÃƒÂ©dio', 'Custo total']} rows={items.data.map(row => [String(row.item_name), `${row.quantity} ${row.unit_of_measure}`, money(row.average_cost), money(row.cost)])} /></Shell>;
}

export function FranchiseChecklists() {
  const result = useLoad(franchisePortalService.checklistOccurrences);
  return <State result={result}>{response => <Shell title="Checklists" description="OcorrÃƒÂªncias e execuÃƒÂ§ÃƒÂ£o operacional da unidade."><Table headers={['Checklist', 'Data', 'Prazo', 'Status']} rows={response.data.map(row => [String((row.template as Record<string, unknown>)?.name ?? 'Ã¢â‚¬â€'), String(row.scheduled_date), String(row.due_at ?? 'Ã¢â‚¬â€'), String(row.status)])} /></Shell>}</State>;
}

export function FranchiseTrainings() {
  const result = useLoad(franchisePortalService.trainings);
  return <State result={result}>{response => <Shell title="Treinamentos" description="ConteÃƒÂºdos atribuÃƒÂ­dos a vocÃƒÂª ou ÃƒÂ  sua unidade."><Table headers={['Treinamento', 'Categoria', 'DuraÃƒÂ§ÃƒÂ£o', 'ObrigatÃƒÂ³rio', 'Status']} rows={response.data.map(row => [String(row.title), String(row.category ?? 'Ã¢â‚¬â€'), `${row.duration_minutes ?? 0} min`, row.mandatory ? 'Sim' : 'NÃƒÂ£o', String(row.status)])} /></Shell>}</State>;
}

export function FranchiseDocuments() {
  const result = useLoad(franchisePortalService.documents);
  return <State result={result}>{response => <Shell title="Documentos" description="Arquivos disponibilizados para o portal da unidade."><Table headers={['Documento', 'Categoria', 'Tipo', 'Data', 'Download']} rows={response.data.map(row => [String(row.title), String(row.category_name ?? 'Ã¢â‚¬â€'), String(row.extension ?? 'Ã¢â‚¬â€').toUpperCase(), String(row.created_at ?? 'Ã¢â‚¬â€').slice(0, 10), <button onClick={() => void franchisePortalService.downloadDocument(String(row.id), String(row.file_name ?? row.title)).catch(error => toast.error(error.message))} style={{ border: 0, background: '#EFF6FF', color: '#2563EB', borderRadius: 7, padding: '6px 9px', cursor: 'pointer' }}><Download size={13} /></button>])} /></Shell>}</State>;
}

export function FranchiseContracts() {
  const result = useLoad(franchisePortalService.contracts);
  return <State result={result}>{response => <Shell title="Contratos" description="Contratos vinculados ÃƒÂ  sua unidade."><Table headers={['Contrato', 'NÃƒÂºmero', 'Cliente', 'InÃƒÂ­cio', 'Fim', 'Status']} rows={response.data.map(row => [String(row.title), String(row.contract_number ?? 'Ã¢â‚¬â€'), String(row.customer_name ?? 'Ã¢â‚¬â€'), String(row.start_date ?? 'Ã¢â‚¬â€'), String(row.end_date ?? 'Ã¢â‚¬â€'), String(row.status)])} /></Shell>}</State>;
}

export function FranchiseTasks() {
  const result = useLoad(franchisePortalService.tasks);
  return <State result={result}>{response => <Shell title="Tarefas" description="AÃƒÂ§ÃƒÂµes atribuÃƒÂ­das a vocÃƒÂª ou ÃƒÂ  sua unidade."><Table headers={['Tarefa', 'Prioridade', 'Origem', 'Prazo', 'Status', 'AÃƒÂ§ÃƒÂ£o']} rows={response.data.map(row => [
    String(row.title), String(row.priority), String(row.source_type ?? 'manual'), String(row.due_date ?? 'Ã¢â‚¬â€'), String(row.status),
    <select key={`status-${row.id}`} value={String(row.status)} onChange={event => void franchisePortalService.updateTaskStatus(String(row.id), event.target.value as 'open' | 'in_progress' | 'completed').then(() => result.load()).catch(() => toast.error('NÃƒÂ£o foi possÃƒÂ­vel atualizar a tarefa.'))}><option value="open">Aberta</option><option value="in_progress">Em andamento</option><option value="completed">ConcluÃƒÂ­da</option></select>,
  ])} /></Shell>}</State>;
}
...
export function FranchiseInventory() {
  ...
  return <Shell title="Estoque" description="Saldos por local e disponibilidade operacional da sua unidade.">
    <Table headers={['Item', 'Saldo', 'Reservado', 'Bloqueado', 'DisponÃƒÂ­vel', 'MÃƒÂ­nimo']} rows={stock.data.stock.map(row => [String(row.item_name), `${row.on_hand ?? 0} ${row.unit_of_measure}`, String(row.reserved ?? 0), String(row.blocked ?? 0), String(row.available ?? 0), settings.enable_stock_minimum ? String(row.minimum_stock) : 'Ã¢â‚¬â€'])} />
  </Shell>;
}
