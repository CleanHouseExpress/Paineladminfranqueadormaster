...
export function TrainingSettings() {
  ...
  const [singularLabel, setSingularLabel] = useState('Treinamento');
  const [pluralLabel, setPluralLabel] = useState('Treinamentos');
  ...
  useEffect(() => {
    async function load() {
      const response = await apiClient.get<MetadataResponse>('/api/metadata/trainings');
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const response = await apiClient.put<MetadataResponse>('/api/metadata/trainings', {
    ...
    <div style={{ padding: '28px 32px', minHeight: '100vh', background: '#F8FAFC' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#0F172A', margin: '0 0 4px' }}>Configuracoes de Treinamentos</h1>
            <p style={{ margin: 0, fontSize: '14px', color: '#64748B' }}>Personalize labels, campos e exibicao do modulo</p>
          </div>
        </div>
        <button onClick={() => navigate('/trainings')} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '10px', border: '1.5px solid rgba(0,0,0,0.1)', background: '#fff', color: '#64748B', fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {activeSection === 'campos-visiveis' && (
            <div style={cardStyle}>
              ...
              <p style={subtitleStyle}>Defina quais campos aparecem no formulario de treinamento.</p>
              {fields.map(field => <Row key={field.key} title={field.label} subtitle={field.key}><Toggle checked={field.visible !== false} onChange={() => updateField(field.key, { visible: field.visible === false })} disabled={field.key === 'title'} /></Row>)}
            </div>
          )}
          ...
          {activeSection === 'campos-obrigatorios' && (
            <div style={cardStyle}>
              ...
              {fields.map(field => {
                const locked = field.key === 'title';
                return <Row key={field.key} title={field.label} subtitle={field.key}><Toggle checked={locked || field.required === true} onChange={() => updateField(field.key, { required: !field.required })} disabled={locked} /></Row>;
              })}
            </div>
          )}
          ...
          {activeSection === 'colunas' && (
            <div style={cardStyle}>
              ...
              {columns.map(column => <Row key={column.key} title={column.label} subtitle={column.key}><Toggle checked={column.visible !== false} onChange={() => updateColumn(column.key, { visible: column.visible === false })} disabled={column.key === 'title'} /></Row>)}
            </div>
          )}
...
function inputStyle(): React.CSSProperties {
  return { display: 'block', width: '100%', marginTop: '6px', border: '1.5px solid rgba(0,0,0,0.1)', borderRadius: '8px', padding: '8px 12px', fontSize: '13px', color: '#0F172A', outline: 'none', background: '#fff', boxSizing: 'border-box' };
}

function smallButton(): React.CSSProperties {
  return { width: '28px', height: '28px', borderRadius: '8px', border: '1px solid rgba(0,0,0,0.1)', background: '#fff', color: '#64748B', cursor: 'pointer' };
}
